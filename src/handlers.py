...
async def button_show_relevant_links(message: Message, bot: Bot, state: FSMContext):
    data = await state.get_data()
    response = data['response']
    text = service_messages.show_relevant_links()
    await bot.send_message(chat_id=message.chat.id, text=text, reply_markup=keyboards.list_of_relevant_links(response))